...
// Executes a command by forking and running it in a child processs
...
// Attemps to execute the command by searching paths and the current directory
int child_process(char** args,char** env){

    char* path_string = get_path(env);
    int num_path;
    char** path_list = split_paths(path_string,&num_path);
...
// fetches the path from the environment variables
...
// Split the PATH string int individual paths
